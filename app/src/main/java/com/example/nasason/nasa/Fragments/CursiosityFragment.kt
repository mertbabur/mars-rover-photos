...
import kotlinx.android.synthetic.main.fragment_cursiosity2.view.*


class CursiosityFragment : Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val rootView: View = inflater.inflate(R.layout.fragment_cursiosity2, container, false)

        defineAttributes(rootView)
        ToolbarSet.defineToolbar(activity, rootView.toolbarForCursiosity)
        setHasOptionsMenu(true)
        getArgumentsForCameraType()
        getPhotoFromApi(Const.CURSIOSITY, 0)
        loadMorePhotoFromApi(Const.CURSIOSITY)
    ...
    fun defineAttributes(rootView: View){
        recyclerView = rootView.recyclerViewForCursiosity
        progressBar = rootView.progressBarForCursiosity
        scrollView = rootView.nestedScrollViewForCursiosity
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        inflater.inflate(R.menu.menu_for_toolbar_curiosity,menu)
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        val fragment = CursiosityFragment()
        return when(item.itemId){
            R.id.action_allForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.ALL_CAMERA)
                true
            }
            R.id.action_fhazForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.FHAZ_CAMERA)
                true
            }
            R.id.action_rhazForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.RHAZ_CAMERA)
                true
            }
            R.id.action_mastForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.MAST_CAMERA)
                true
            }
            R.id.action_chemcamForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.CHEMCAM_CAMERA)
                true
            }
            R.id.action_mahliForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.MAHLI_CAMERA)
                true
            }
            R.id.action_mardiForCuriosity -> {
                NewFragment.replaceFragmet(activity, fragment, Const.BUNDLE_KEY, Const.MARDI_CAMERA)
                true
            }
            R.id.action_navcamForCuriosity -> {
